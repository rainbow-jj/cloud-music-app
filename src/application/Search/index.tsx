...
import { Container, ShortcutWrapper, HotKey, ListItem , List, SongItem} from './style';
...
function Search(props) {
  ...
  //由于是传递给子组件的方法，尽量用useCallback 包裹，以使得依赖未改变，始终传给子组件的说是相同的引用
            ...
            return (
              <ListItem key={item.accountId + "" + index} onClick={() =>  props.history.push(`/singers/${item.id}`) }>
                <div className="img_wrapper">
                  <LazyLoad placeholder={<img src={require('./singer.png')} width="100%" height="100%" />}>
              ...
              <ListItem key={item.accountId + "" + index} onClick={() => {props.history.push(`/album/${item.id}`)}}>
                <div className="img_wrapper">
                  <LazyLoad placeholder={<img width="100%" height="100%" src={require('./music.png')} alt="music"></img>}> 
                    <img src={item.coverImgUrl} width="100%" height="100%" alt="music"></img>
                  </LazyLoad>
                </div>
